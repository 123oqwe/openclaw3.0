...
function unavailableRefresh(record: OutcomeRecord, observedAt: number, reason: RefreshReason): RefreshCandidate {
  ...
  const projections = record.criteria
    ...
    .filter((ref, index, refs) => refs.findIndex((item) => workRefIdentity(item) === workRefIdentity(ref)) === index)
    .map((ref) => ({ ref, availability, observedAt, proofs: [], artifacts: [], errorCode: reason }));
  return {
    projections,
    evidence: [],
    authorizedSources: [],
...
function buildRefreshCandidate(record: OutcomeRecord, cards: Awaited<ReturnType<typeof readAuthorizedWorkboardCards>>, observedAt: number): RefreshCandidate {
  ...
  const refs = record.criteria
    .flatMap((criterion) => criterion.workRefs)
    .filter((ref, index, all) => all.findIndex((item) => workRefIdentity(item) === workRefIdentity(ref)) === index);
  ...
  for (const ref of refs) {
    ...
    if (candidates.length !== 1 || exact.length !== 1) {
      unavailable.set(workRefIdentity(ref), candidates.length === 0 ? "not-found" : "identity-conflict");
      continue;
    }
  ...
  const projections = refs.map((ref) => {
    ...
    if (reason !== undefined) {
      return {
        ref,
        availability: reason === "identity-conflict" ? ("identity-conflict" as const) : ("unavailable" as const),
        observedAt,
        proofs: [],
        artifacts: [],
        errorCode: reason,
...
export function registerOutcomeFirstPackageMethods(api: OpenClawPluginApi): void {
  ...
  api.registerGatewayMethod("outcomes.refresh", async ({ client, params, respond }) => {
